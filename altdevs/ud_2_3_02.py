...
This time, only 2 bins but with the challenge
of keeping the combinatorics under overflow.
Cutpoints are still at +/- eps of data
...
# ~ def addend_std(n, i):
    # ~ return comb(n, i) * (i/n)**i * ((n-i)/n)**(n-i)

# ~ def addend(n, i):
    # ~ return float(comb(n, i) * Fraction(i, n)**i * Fraction(n-i, n)**(n-i))

# ~ def initreg(n):
    # ~ reg = [0, 1]
    # ~ reg.append( sum( addend(n, i) for i in range(n+1) ) )
    # ~ for k3 in range(5):
        # ~ "complete up to 7 values, k3 is k-3 actually"
        # ~ reg.append(reg[-1] + (n/(k3+1)*reg[-2]))
    # ~ return reg
...
# ~ for i in range(8):
    # ~ print(300 + i*100, ' '.join(f"{r:.3f}" for r in initreg_std(300 + i*100)))
    # ~ print(300 + i*100, ' '.join(f"{r:.3f}" for r in initreg(300 + i*100)))
...
# Titanic edge labels w/o zeros, no duplicities:
d = [0, 6, 23, 24, 45, 52, 57, 64, 79, 106, 118, 122, 126, 145, 167, 178, 179, 180, 196, 203, 212, 261, 319, 344, 367, 425, 510, 528, 627, 654, 673, 862, 885, 1364, 1438, 1667]

eps = 0.1

# I BELIEVE I AM FOOLING MYSELF WITH THESE TITANIC LABELS

# candcuts[0] and candcuts[-1] always belong to the cut sequence
candcuts = tuple(chain.from_iterable([a - eps, a + eps] for a in d))

print("len candcuts", len(candcuts), '\n    ',
      "log comb 1", log(1/(len(candcuts)-2)),  '\n    ',
      "log comb 2", log(1/comb(len(candcuts)-2, 2)))

ll, oc = ocut(candcuts, tuple(d), len(candcuts) - 1)

print(ll, oc)

for j in range(1, 6):
    print(j, oc, len(candcuts) - j, candcuts[len(candcuts) - j])
